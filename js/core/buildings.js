const BUILDINGS = {
	[SPECIAL_CHARS.tri]: {
		...
		name: "Drill v1",
		getProduction(b) {
			return map[b.pos.x][b.pos.y][1].sub(9.999e6).pow(1/3).mul(0.1)
		},
		canPlace(x, y) {
	...
	[SPECIAL_CHARS.theta]: {
		...
		currencyDisplayName: "_",
		currencyInternalName: "shards",
		desc: `Clears unexplored tiles in a <b>7x7</b> area<br>
...
function startBuildingData(node, x, y) {
	switch (node) {
		case SPECIAL_CHARS.tri:
			return map[x][y][1].sub(9.999e6);
		case 'x':
			return {
				active: false,
				food: D(0)
			};
		case SPECIAL_CHARS.theta:
			return D(0);
	}
}
function stopPlacing() {
	...
	player.buildings[placeData.node].push({pos: {x, y}, meta: startBuildingData(placeData.node, x, y)});
...
function sell(x, y, type) {
	Modal.close();
	let b = BUILDINGS[type];
	player.buildings[type].splice(getBuildingWPos(x, y, type, true), 1);
	...
	player.attributes.powerUsed = player.attributes.powerUsed.sub(building.power);
	map[x][y][0] = getMapEmpty(x, y);
	render();
	renderLayer2();
}
function getBuildingWPos(x, y, type, id=false) {
	if (id) {
...
function loadBuildings() {
	Vue.component("building-ui", {
		...
		template: `<div class="building-segment" @click="startPlacing(bId, type)">
