...
const Magic = {
  Enchants: {
    ...
    buy(id) {
      const enc = Magic.Enchants.LIST[id];
  ...
  Spells: {
    ...
    buy(id) {
      const spell = Magic.Spells.LIST[id];
    ...
    load() {
      Vue.component("spell", {
        ...
        template: `<button :class="{
          ...
          <b style="font-size: 17px; margin-bottom: 2px;" 
            v-html="spell.name + ' (' + format(player.spells[sId].time, 1) + 's)'"></b>
